...
\end{equation}
Since this function outputs values between zero and one and the sum of $x$ is equal one \code{($\sum_i f(x_i)=1$)}
...
\begin{figure}[ht]
    ...
    %\caption{\gls{ReLU} activation function: $max(0,x)$ and \\SELU activation function $max(\alpha\cdot e^x-\alpha,
    x)$} \label{fig:ReLU}
    \begin{tikzpicture}
        \begin{axis}[
    axis lines=middle,
