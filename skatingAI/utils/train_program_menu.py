...
class TrainProgram(object):
    ...
    def create_menu(self) -> (
        ...
        general_train_params = GeneralTrainParams(
            gpu=1, wcounter_hp=-1, wcounter_kps=-1, epoch_start=0, batch_size=2,
            epochs=12, epoch_steps=2, epoch_log_n=1
        )
        train_hp, train_kps = False, False
        print('\n' * 2)
        print('Welcome to the custom training of body parts and key points of skatingAI ⛸💡')

        title = f'Do you want to stick to the default parameters for training?'
        ...
        if index == 0:
            ...
            gpu = input('Please choose a gpu to train on [1,2,3]:')
            general_train_params.gpu = int(gpu) if self._parse_if_int(gpu) in [1, 2, 3] else general_train_params.gpu
            wcounter_hp = input('Please choose a weight counter index for the body parts pre-training:')
            general_train_params.wcounter_hp = int(wcounter_hp) if self._parse_if_int(
                wcounter_hp) else general_train_params.wcounter_hp
            wcounter_kps = input('Please choose a weight counter index for the keypoints pre-training:')
