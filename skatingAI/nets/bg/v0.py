...
class BGNet(BGNetBase):
    ...
    def _build_model(self) -> tf.keras.Model:
        ...
        block_l = self.conv3x3_block(input, 2, filter_counts=[16, 16, 36],
                                     layer_type=['c', 'c', 'c'], name="bl")
        block_m = self.stride_down(self.inputs, name="bm")
        block_m = self.conv3x3_block(block_m, 2, filter_counts=[64, 36, 36],
                                     layer_type=['c', 'c', 'c'], name="bm")
        block_m = self.stride_up(block_m, 2, name="bm")

        add = layers.add([block_l, block_m])

        concat = layers.concatenate([add, input])

        block_l = self.conv3x3_block(concat, 4, filter_counts=[16, 16, 36],
                                     layer_type=['c', 'c', 'c'], name="bl")
        # --------------third-block-------------------#
